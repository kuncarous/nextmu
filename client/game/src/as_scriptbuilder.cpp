...
int CScriptBuilder::AddSectionFromFile(const char *filename)
{
	...
	string fullpath = ResolveToRelativePath(filename);
...
int CScriptBuilder::LoadScriptSection(const char *filename)
{
	...
	if (mu_rwfromfile<EGameDirectoryType::eSupport>(&fp, filename, "rb") == false)
	{
		...
		string msg = "Failed to open script file '" + ResolveToRelativePath(scriptFile) + "'";
	...
	mu_isize len = static_cast<mu_isize>(SDL_RWsize(fp));

	size_t c = 0;
	std::unique_ptr<mu_char[]> buffer(new_nothrow mu_char[len + 1]);
	buffer[len] = '\0';

	// Read the entire file
	if (len > 0)
	{
		c = SDL_RWread(fp, buffer.get(), len, 1);
	}
	SDL_RWclose(fp);
	...
	{
		// Write a message to the engine's message callback
		string msg = "Failed to load script file '" + ResolveToRelativePath(scriptFile) + "'";
		engine->WriteMessage(filename, 0, 0, asMSGTYPE_ERROR, msg.c_str());
		return -1;
	}

	// Process the script section even if it is zero length so that the name is registered
	return ProcessScriptSection(buffer.get(), (unsigned int)(len), filename, 0);
}
