...
namespace UIRmlUI
{
    NSystemInterface::NSystemInterface()
    {
    ...
    void NSystemInterface::SetWindow(SDL_Window* in_window)
    {
        window = in_window;
    }
    ...
    void NSystemInterface::SetClipboardText(const Rml::String& text_utf8)
    {
        SDL_SetClipboardText(text_utf8.c_str());
    }

    void NSystemInterface::GetClipboardText(Rml::String& text)
    {
    ...
    Rml::Input::KeyIdentifier ConvertKey(mu_int32 sdlkey)
    {
        // clang-format off
        switch (sdlkey)
        {
    ...
    mu_int32 ConvertMouseButton(mu_int32 button)
    {
        switch (button)
        {
