...
void main()
{
	...
	position.z = float(texelFetch(s_heightTexture, ivec2(a_position), 0).x) * 1.5;
	
	vec3 normal = texelFetch(s_normalTexture, ivec2(a_position), 0).xyz;
	v_color0 = vec4(texelFetch(s_lightTexture, ivec2(a_position), 0).xyz * clamp(dot(normal, vec3(0.5, -0.5, 0.5)) + 0.5, 0.0, 1.0), 1.0);
	v_color0 *= visible;
	
	v_setting = vec3(texelFetch(s_mappingTexture, ivec2(a_texcoord0), 0).xy, 0.0);
	v_setting.z = float(texelFetch(s_mappingTexture, ivec2(a_position), 0).z) / 255.0;
	
	vec4 uv1 = texelFetch(s_uvTexture, ivec2(v_setting.x, 0), 0);
	vec4 uv2 = texelFetch(s_uvTexture, ivec2(v_setting.y, 0), 0);
	
	v_texcoord0 = vec4(a_position.xyxy) * vec4(uv1.xy, uv2.xy);
	
	vec2 waterMove = vec2(u_terrainSettings.x, sin(u_terrainSettings.z + float(a_position.x) * 5.0) * u_terrainSettings.y * 0.002);
